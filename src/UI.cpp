...
void UI::resetInteracts()
{
	...
	interacts.playerInteraction = PlayerInt::None;
}
...
Interactions UI::inputHandler(int& window, optional<shared_ptr<int>> event)
{
	...
	//if (const auto* keyPressed = event->getIf<sf::Event::KeyPressed>())
	{
		//if (keyPressed->scancode == sf::Keyboard::Scancode::D)
		{
			...
			interacts.playerInteraction = PlayerInt::Draw;
			return interacts;
		}
	}
		...
		//if (mbPressed->button == sf::Mouse::Button::Left)
		{
			...
			/*for (auto& button : buttons)
			{
					...
					button.getName() == "play")
				{
					...
					interacts.playerInteraction = PlayerInt::Play;
...
void UI::layoutHandler(int& window, Snapshot snapshot)
{
	CardState state;
...
void UI::drawHandler(int& window)
{
